...
option_list = list(
  make_option(c("-n", "--num_loans_to_read"), type="integer", default=1e5, 
...
for (k in 1:numberofloops) {
  ...
  while (iter < length(possible.start.pts) & length(ids.added.so.far) < opt$num_loans_to_read) {
    ...
    # create DID_DFLT and DID_PREPAY
    id.codes[, c("DID_DFLT", "DID_PREPAY") := 
               list(ifelse(V1 %in% CreditEventCodes, 1, 0),
                    ifelse(V1 == '01', 1, 0))]
    # remove loans that did not default but
    # zero-balanced before last date (<1% of loans)
    invalid.ids <- id.codes[(DID_DFLT == 0 & !(V1 %in% c("", "01")))]$LOAN_ID
    ...
    Data.Last.Valid <- rbindlist(list(Data.Last.Valid, 
                                      id.codes[!(id.codes$LOAN_ID %in% invalid.ids),
                                               c('LOAN_ID', 'DID_DFLT', 'DID_PREPAY')]), use.names=TRUE)
    iter <- iter + 1
  }
