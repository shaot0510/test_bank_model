...
for trial_i in range(20):
    ...
    modified_vars = [v for v in train_rf.columns
                     if v not in other + ['AGE', 'Intercept']]
    # remove intercept
    ...
    test_coxt = test_rf.copy()

    # drop unnecessary cols
    test_rf.drop(['PRD', 'ORIG_DTE'], axis=1, inplace=True)

    # drop NAs
    a = test_rf.shape
    test_rf = test_rf.dropna(axis=0, how='any')
    print('{0} -> {1}'.format(a, test_rf.shape))
    ...
    plt.legend()
    # plt.show()
    plotname = '.'.join([str(trial_i), 'cum_sum_comparison_RF_COX',
                         filename, 'eps'])
